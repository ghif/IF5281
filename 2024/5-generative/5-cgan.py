...
class Generator(nn.Module):
    ...
    """
    def __init__(self, input_size=100, condition_size=10, output_size=784):
        super().__init__()
    ...
    def forward(self, z, c):
        z, c = z.view(z.size(0), -1), c.view(c.size(0), -1).float()
        v = torch.cat((z, c), 1) # v: [latent, conditions] concatenated vector
        x = self.layer(v)
...
netG = Generator(input_size=NZ, condition_size=num_classes, output_size=dx1 * dx2).to(DEVICE)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, c):
        ...
        v = torch.cat((x, c), 1) # v: [latent, conditions] concatenated vector
        y = self.layer(v)
        y = torch.flatten(y)
        return y
...
fixed_y = torch.tensor([i % 9 for i in range(BATCH_SIZE)])
fixed_cvec = F.one_hot(fixed_y, num_classes=num_classes).to(DEVICE)
...
for epoch in range(NUM_EPOCH):
    ...
    for batch_idx, (X, y) in enumerate(train_loader):
        ...
        c_vec = F.one_hot(y, num_classes=num_classes).to(DEVICE)
        ...
        fake_x = netG(noisy_latent, c_vec)
        label.fill_(fake_label)
        fake_x = torch.flatten(fake_x, start_dim=1)
        outputD = netD(fake_x, c_vec)
        ...
        if batch_idx % 100 == 0:
            ...
            real_x = real_x.view(-1, NC, dx1, dx2)
            vutils.save_image(real_x, f'{sample_dir}/real_samples.jpg', normalize=True)
            fixed_fake = netG(fixed_latent, fixed_cvec)
